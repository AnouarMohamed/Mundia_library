...
| **Catalog Metadata** | Next.js remains authoritative for `books`; Catalog has secured PostgreSQL reads, non-routed idempotent metadata commands, crash-recoverable Protobuf outbox delivery, and strict Circulation availability/returned-loan projection consumption, while review commands, replay evidence, backfill, BFF routing, and cutover remain pending | Serviced exclusively by `Catalog Service` via [contracts/catalog.ts](../lib/services/contracts/catalog.ts) | Grep scan for direct `books` table queries |
