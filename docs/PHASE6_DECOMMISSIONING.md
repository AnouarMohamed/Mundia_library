...
| **Catalog Metadata** | Next.js remains production-routed for `books` and reviews; Catalog has secured PostgreSQL reads, non-routed idempotent metadata and review commands, crash-recoverable Protobuf outbox delivery, and strict Circulation availability/returned-loan projection consumption, while replay evidence, backfill, BFF routing, and cutover remain pending | Serviced exclusively by `Catalog Service` via [contracts/catalog.ts](../lib/services/contracts/catalog.ts) | Grep scan for direct `books` and review table queries |
