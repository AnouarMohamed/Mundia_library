...
| **Catalog Metadata** | Next.js remains authoritative for `books`; Catalog has secured PostgreSQL reads plus non-routed idempotent create, metadata-update, and edition-activation commands, while review commands, outbox delivery, availability events, backfill, BFF routing, and cutover remain pending | Serviced exclusively by `Catalog Service` via [contracts/catalog.ts](../lib/services/contracts/catalog.ts) | Grep scan for direct `books` table queries |
