...
| **Catalog Metadata** | Next.js remains authoritative for `books`; Catalog has a secured PostgreSQL-backed read slice, but writes, availability events, backfill, BFF routing, and cutover remain pending | Serviced exclusively by `Catalog Service` via [contracts/catalog.ts](../lib/services/contracts/catalog.ts) | Grep scan for direct `books` table queries |
