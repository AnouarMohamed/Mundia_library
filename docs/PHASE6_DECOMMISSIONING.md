...
| **Catalog Metadata** | Next.js remains authoritative for `books`; Catalog has secured PostgreSQL reads, non-routed idempotent metadata commands, and crash-recoverable Protobuf outbox delivery, while review commands, availability events, backfill, BFF routing, and cutover remain pending | Serviced exclusively by `Catalog Service` via [contracts/catalog.ts](../lib/services/contracts/catalog.ts) | Grep scan for direct `books` table queries |
