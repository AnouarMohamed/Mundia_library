...
| Circulation integrity | Legacy mutations are conditional; the target command slice has database constraints, caller-bound loan/inventory/fine idempotency, deterministic copy locking, immutable fine ledger, an atomic outbox, published OpenAPI/Protobuf contracts, and PostgreSQL 18 concurrency tests | Reservation/policy completion, consumer inbox recovery, membership eligibility, reconciliation, production-shaped load, failure injection, and cutover evidence |
