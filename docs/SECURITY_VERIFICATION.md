...
| Circulation integrity | Legacy mutations are conditional; the target command slice has database constraints, caller-bound idempotency, deterministic copy locking, an atomic outbox, and PostgreSQL 18 concurrency tests | Renewal/fine completion, relay/inbox recovery, membership eligibility, reconciliation, production-shaped load, failure injection, and cutover evidence |
