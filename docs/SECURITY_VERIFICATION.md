...
| Container             | All four application images use digest-pinned bases and unprivileged runtimes; gated GHCR publication emits multi-architecture manifests, SBOMs, provenance, and immutable digest artifacts       | Signature enforcement, registry/admission enforcement, runtime hardening evidence, and deployed scan evidence                                          |
| CI security           | Pinned Actions, JS/TS and Kotlin/Java CodeQL, dependency review, npm audit, secret scanning, migration/concurrency gates, and blocking high/critical scans for all four release images are configured | IaC/Kubernetes policy scans, keyless image signing, retained production run evidence, and protected branch/environment policy                          |
