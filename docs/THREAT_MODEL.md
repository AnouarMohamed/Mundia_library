...
| TM-07 | T, R    | Concurrent or replayed approve/reject/cancel/return/renew commands produce duplicate loans, impossible state, or incorrect fines    | Explicit state machine; conditional update/version; copy-level uniqueness; serializable/locked transaction; idempotency key; immutable ledger; reconciliation             | 100-way concurrency tests, duplicate request replay, property/state-machine tests, database constraint evidence |
