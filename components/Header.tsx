...
const Header = async ({ session }: HeaderProps) => {
  ...
  return (
    <header className="sticky top-3 z-40 mb-6 sm:mb-8">
      <div className="rounded-[1.35rem] border border-white/10 bg-[rgba(5,15,24,0.78)] px-4 py-3 shadow-[0_20px_60px_rgba(2,8,13,0.34)] backdrop-blur-xl sm:px-6 sm:py-4">
        <div className="flex items-center justify-between gap-3">
          <Link href="/" className="group inline-flex min-w-0 items-center gap-2 sm:gap-3">
            <span className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-white/15 bg-white/10 shadow-inner sm:size-12">
              <img src="/icons/logo.svg" alt="logo" width={40} height={40} className="size-7 sm:size-8" />
            </span>
            <div className="min-w-0">
              <p className="truncate font-bebas-neue text-2xl leading-none tracking-[0.12em] text-white sm:text-3xl">
                Mundiapolis
              </p>
              <p className="truncate text-[10px] uppercase tracking-[0.28em] text-[#d6b86d] sm:text-xs">
                University Library
              </p>
            </div>
          </Link>
          ...
          <ul className="hidden items-center gap-1 md:flex lg:gap-2">
            {navLinks.map((link) => (
              <li key={link.href}>
                <Link
                  href={link.href}
                  className="rounded-full px-3 py-2 text-sm font-medium text-light-100/85 transition hover:bg-white/10 hover:text-white lg:px-4"
                >
                  {link.label}
                </Link>
              </li>
