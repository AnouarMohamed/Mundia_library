...
class CatalogCommandBodyLimitFilter : OncePerRequestFilter() {
    override fun shouldNotFilter(request: HttpServletRequest): Boolean =
        request.method !in BODY_METHODS || !request.requestURI.startsWith(CATALOG_API_PREFIX)
    ...
    private fun writePayloadTooLarge(response: HttpServletResponse) {
        ...
        response.writer.write(
            """{"type":"urn:mundia:error:payload_too_large","title":"Payload Too Large","status":413,"detail":"Catalog command body exceeds 131072 bytes","code":"payload_too_large"}""",
        )
    }
    ...
    private companion object {
        const val CATALOG_API_PREFIX = "/api/v1/catalog/"
        const val MAX_COMMAND_BODY_BYTES = 128 * 1024
