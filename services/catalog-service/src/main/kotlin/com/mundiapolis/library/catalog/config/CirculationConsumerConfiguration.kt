...
class CirculationConsumerConfiguration {
    @Bean(destroyMethod = "")
    @ConditionalOnProperty(prefix = "app.circulation-consumer", name = ["enabled"], havingValue = "true")
    fun circulationKafkaClient(properties: CirculationConsumerProperties): Consumer<String, ByteArray> {
        ...
        val configuration = mutableMapOf<String, Any>(
            ...
            ConsumerConfig.MAX_PARTITION_FETCH_BYTES_CONFIG to properties.maximumEventBytes + RECORD_OVERHEAD_BYTES,
            ConsumerConfig.FETCH_MAX_BYTES_CONFIG to
                (properties.maximumEventBytes + RECORD_OVERHEAD_BYTES) * properties.maximumPollRecords,
            ...
            ConsumerConfig.HEARTBEAT_INTERVAL_MS_CONFIG to kafka.heartbeatInterval.toMillis().toInt(),
    ...
    @Bean
    @ConditionalOnProperty(prefix = "app.circulation-consumer", name = ["enabled"], havingValue = "true")
    fun circulationEventRecordDecoder(
        properties: CirculationConsumerProperties,
    ): CirculationEventRecordDecoder = CirculationEventRecordDecoder(properties)

    @Bean
    @ConditionalOnProperty(prefix = "app.circulation-consumer", name = ["enabled"], havingValue = "true")
    fun circulationAvailabilityKafkaConsumer(
    ...
    ): CirculationAvailabilityKafkaConsumer = CirculationAvailabilityKafkaConsumer(
        consumer,
        decoder,
        projectionService,
    ...
    @Bean
    fun catalogCirculationConsumerHealthIndicator(
        consumer: ObjectProvider<CirculationAvailabilityKafkaConsumer>,
        properties: CirculationConsumerProperties,
        clock: Clock,
        ...
        builder.withDetail("enabled", true)
            ...
            .apply { snapshot.failure?.let { withDetail("failure", it.name) } }
            .build()
    }
