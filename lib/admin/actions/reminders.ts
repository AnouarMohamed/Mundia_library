...
export class EmailService {
  ...
  private static generateEmailTemplate(subject: string, body: string): string {
    ...
    <body>
        <div class="container">
            <div class="header">
                <div class="logo"> Mundiapolis Library</div>
            ...
            <div class="footer">
                <p><strong>Mundiapolis University Library Management System</strong></p>
                <div class="contact-info">
                    <p> Email: support@mundiapolis-library.edu</p>
                    <p> Phone: +55 (11) 3000-0000</p>
                    <p> Website: library.mundiapolis.edu.br</p>
                    <p> Address: Mundiapolis University, São Paulo, SP, Brazil</p>
                </div>
...
export async function getBooksDueSoon() {
  ...
  const dueSoonBooks = await db
    .select({
      ...
      daysUntilDue: sql<number>`(${borrowRecords.dueDate} - CAST(${startOfToday} AS date))`,
    })
    .from(borrowRecords)
...
export async function getOverdueBooks() {
  ...
  const overdueBooks = await db
    .select({
      ...
      borrowDate: borrowRecords.borrowDate,
      dueDate: borrowRecords.dueDate,
      daysOverdue: sql<number>`(CAST(${startOfToday} AS date) - ${borrowRecords.dueDate})`,
...
Thank you for using Mundiapolis Library services.

Best regards,
Mundiapolis Library Management Team

---
This is an automated reminder. For assistance, please contact us at support@mundiapolis-library.edu or visit library.mundiapolis.edu.br.
    `.trim();
...
---
This is an automated notice. For assistance, please contact us at support@mundiapolis-library.edu or visit library.mundiapolis.edu.br.
    `.trim();
